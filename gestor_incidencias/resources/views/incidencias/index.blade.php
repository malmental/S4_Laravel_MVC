    ...
    <div class="divide-y divide-gray-300">
        ...
        <div class="grid grid-cols-12 gap-4 px-6 py-4 hover:bg-cream-dark items-center">
            ...
            <div class="col-span-3 flex gap-2 justify-end">
                <a href="{{ route('incidencias.edit', $incidencia->id) }}" class="px-4 py-1 border-2 border-black bg-white text-xs hover:bg-cream-dark">Editar</a>
                <form action="{{ route('incidencias.destroy', $incidencia->id) }}" method="POST" class="inline">
                        @csrf @method('DELETE')
                    <button type="submit" class="px-3 py-1 border-2 border-black bg-black text-white text-xs hover:bg-gray-800" onclick="return confirm('¿Seguro que deseas eliminar esta incidencia?')">X</button>
                </form>
            </div>
        </div>
